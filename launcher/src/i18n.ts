...
const en = {
  ...
  evenBiggerContext: "Even Bigger Context™ (super experimental)",
  evenBiggerContextBody: "Doubles Bigger Context to 6× logical context (540k hard / 480k auto-compact on Plus Medium/High) and may use up to eight staged browser messages. ChatGPT may truncate or compress older staged context, and the extra requests can increase rate limits or cooldowns. Requires Bigger Context.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  evenBiggerContext: "Even Bigger Context™（超级实验性）",
  evenBiggerContextBody: "将 Bigger Context 再翻倍为 6× 逻辑上下文（Plus 的 Medium/High 为 540k 硬上限 / 480k 自动压缩），并可使用最多八条分段浏览器消息。ChatGPT 可能截断或压缩较早的分段上下文，额外请求也可能增加速率限制或冷却。需要先启用 Bigger Context。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  evenBiggerContext: "Even Bigger Context™（超実験的）",
  evenBiggerContextBody: "Bigger Context をさらに 2 倍の 6× 論理コンテキスト（Plus Medium/High で 540k ハード上限 / 480k 自動コンパクション）にし、最大 8 件の段階メッセージを使用します。ChatGPT が古い段階コンテキストを切り捨てまたは圧縮する可能性があり、追加リクエストでレート制限やクールダウンも増える場合があります。Bigger Context が必要です。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  evenBiggerContext: "Even Bigger Context™ (초실험적)",
  evenBiggerContextBody: "Bigger Context를 다시 두 배인 6× 논리 컨텍스트(Plus Medium/High에서 540k 하드 한도 / 480k 자동 컴팩션)로 늘리고 최대 8개의 단계별 브라우저 메시지를 사용할 수 있습니다. ChatGPT가 오래된 단계 컨텍스트를 자르거나 압축할 수 있으며 추가 요청으로 속도 제한이나 쿨다운이 늘 수 있습니다. Bigger Context가 필요합니다.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  evenBiggerContext: "Even Bigger Context™（超級實驗性）",
  evenBiggerContextBody: "將 Bigger Context 再加倍為 6× 邏輯上下文（Plus Medium/High 為 540k 硬上限 / 480k 自動壓縮），並可使用最多八則分段瀏覽器訊息。ChatGPT 可能截斷或壓縮較舊的分段上下文，額外請求也可能增加速率限制或冷卻。需要先啟用 Bigger Context。",
