...
test("proxies official /models auth and query, then appends one unified ChatGPT Web model", async () => {
  ...
  expect(web.supported_in_api).toBe(true);
  expect(web.priority).toBe(1);
  expect(web.multi_agent_version).toBe("v2");
});
